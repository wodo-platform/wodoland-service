...
export class DemoService {
  ...
  async create(demoCreateDto: DemoCreateDto): Promise<Demo> {
    ...
    const demo: Demo = (await this.prisma.demo.create({
      data,
    })) as Demo;
  ...
  async update(demoUpdateDto: DemoUpdateDto): Promise<Demo> {
    ...
    const demo: Demo = (await this.prisma.demo.update({
      where: {
        id: demoUpdateDto.id,
      },
      data: {
  ...
  async findAll(id: number | null, name: string | null): Promise<Demo[]> {
    const demos: Demo[] = (await this.prisma.demo.findMany({
  ...
  async findById(id: number): Promise<Demo> {
    ...
    const demo: Demo = (await this.prisma.demo.findUnique({
      where: {
        id: id,
      },
    })) as Demo;
    ...
    if (demo) {
      this.logger.debug(
        `found demo[${JSON.stringify(demo)}] in the datastore by demoId[${id}]`,
      );
    } else {
      this.logger.debug(
        `could not find any demo record in the datastore by demoId[${id}]`,
      );
    }
  ...
  async purge(id: number): Promise<Demo> {
    const demo: Demo = (await this.prisma.demo.delete({
      where: {
        id,
      },
    })) as Demo;
