...
import {
  WG_ERROR_INTERNAL_SERVER,
  WG_ERROR_GL_CREATE_VALIDATION,
  WGErrorCode,
  WG_ERROR_GL_UPDATE,
} from '@wodo-platform/wg-shared-lib/dist/wodogaming/error/error.codes';
...
export class ValidationPipe implements PipeTransform<any> {
  ...
  async transform(value, metadata: ArgumentMetadata) {
    ...
    if (
      ...
      this.throwHttpException(
        ...
        this.validationSchema || this.validationSchema === ''
          ? 'Validation schema is not provided in method validator decarator'
          : '[' +
              this.validationSchema +
              '] name provided in method validator decarator is not definet yet.',
      );
    }
    ...
    const validate: any = ajv.getSchema<any>(this.validationSchema);
    const passed = validate(value);
    if (!passed) {
      const errDEtail = this.buildError(validate.errors);
      this.throwHttpException(
        HttpStatus.BAD_REQUEST,
        wgErrorCode,
        'property:' +
  ...
  private buildError(errors) {
    ...
    for (const err of errors as DefinedError[]) {
      const property: any = err.instancePath;
  ...
  private throwHttpException(
    httpStatus: HttpStatus,
    wgErrorCode: WGErrorCode,
    details: string,
  ) {
